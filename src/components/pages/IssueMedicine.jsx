...
const columns = [
  ...
  },
  {
    ...
    selector: (row) => (
      <Link to={`/viewmedicine/${btoa(row.prescriptionid)}`}>
        View Medicine
      </Link>
    ),
  },
    ...
    selector: (row) => (
      <>
        {false ? (
          ...
          <span className="badge bg-danger">Not Issued</span>
        )}
      </>
    ),
  },
...
const IssueMedicine = () => {
  ...
  const filteredItems = data.filter(
    (item) =>
      (item.patient_contact &&item.patient_contact.toLowerCase().includes(filterText.toLowerCase())) ||
      (item.treatmentdate &&item.treatmentdate.toLowerCase().includes(filterText.toLowerCase())) ||
      (item.doctorname &&item.doctorname.toLowerCase().includes(filterText.toLowerCase()))
  );
